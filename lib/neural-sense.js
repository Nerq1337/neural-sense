...
class NeuralSense {
    constructor(settings) {
        this.inputNeurons = [];
        this.hiddenLayers = [];
        this.outputNeuron = new neuron_1.default();
        this.error = 0;
        this.expected = 0;
    ...
    initLayers() {
        ...
        for (let i = 0; i < networkSettings.inputNeurons; i++) {
            this.inputNeurons[i] = new neuron_1.default();
        }
        // Adding a bias neuron
        if (this.networkSettings.bias) {
            this.inputNeurons.push(new neuron_1.default(true));
        }
        ...
        for (let iLayer = 0; iLayer < networkSettings.hiddenLayers.length; ++iLayer) {
            ...
            for (let iNeuron = 0; iNeuron < networkSettings.hiddenLayers[iLayer]; ++iNeuron) {
                this.hiddenLayers[iLayer][iNeuron] = new neuron_1.default();
            }
            if (this.networkSettings.bias) {
                this.hiddenLayers[iLayer].push(new neuron_1.default(true));
            }
        }
    }
    connect() {
    ...
    Train(settings) {
        ...
        neuron_1.default.HyperParams = settings.hyperParams;
